...
try {
    ...
    $eventId = static function (PDO $pdo, int $accountId, string $publicId): int {
        $statement = $pdo->prepare(
            'SELECT id FROM security_audit_events WHERE account_scope=:account AND public_id=:public_id LIMIT 1'
        );
        $statement->execute(['account' => $accountId, 'public_id' => $publicId]);
        $id = $statement->fetchColumn();
        if (!is_numeric($id)) {
            throw new RuntimeException('The security audit event identity could not be resolved.');
        }
        return (int) $id;
    };
    ...
    $assert($disabled['notify_on_emergency_action'] === true, 'Emergency notification preference was lost while promotion was disabled.');
    $expectCode(
        fn () => $preferences->save(
            $accountId,
            $billing['id'],
    ...
    $highEventId = $eventId($pdo, $accountId, $highEvent['public_id']);
    ...
    $caseStatement->execute(['event_id' => $highEventId]);
    ...
    $criticalEvent = $securityAudit->record(
        ...
        metadata: ['scope_hash' => hash('sha256', (string) $accountId)],
        requestId: 'REQ-P32O-CRITICAL-' . $suffix
    );
    $criticalEventId = $eventId($pdo, $accountId, $criticalEvent['public_id']);
    ...
    $caseStatement->execute(['event_id' => $criticalEventId]);
    ...
    $resolutionSummary = 'The integrity mismatch was validated as contained, affected sessions were rotated, and audit-chain verification now passes.';
    $resolutionContext = [
    ...
    $postDisableEventId = $eventId($pdo, $accountId, $postDisableEvent['public_id']);
    ...
    $caseStatement->execute(['event_id' => $postDisableEventId]);
