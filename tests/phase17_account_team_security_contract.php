...
foreach ($required as $path) {
    if (!is_file($root . '/' . $path)) $failures[] = 'Missing Phase 17 file: ' . $path;
}
...
$mfaComplete = $read('public/api/auth/mfa-complete.php');
...
$assert(str_contains($mfa, 'used_at IS NULL LIMIT 1 FOR UPDATE') && str_contains($mfa, 'SET used_at=:now'), 'Recovery codes are not consumed atomically.');
...
$assert(str_contains($mfa, "return ['denied' => 'invalid_code']") && str_contains($mfa, "mfa.challenge_completed', 'denied'"), 'Denied MFA evidence is not committed before rejection.');
$assert(str_contains($mfaComplete, 'attempt_count=attempt_count+1') && str_contains($mfaComplete, 'attempt_count<max_attempts'), 'MFA attempt locking is missing.');
...
$assert(str_contains($team, 'invited_email_normalized') && str_contains($team, 'hash_equals'), 'Invitation acceptance is not bound to the invited email.');
