...
if ($dsn === '' || $username === '' || !is_file($mysqlBinary)) {
...
$run = static function (string $command, string $password): void {
    $descriptor = [
        0 => ['pipe', 'r'],
        1 => ['pipe', 'w'],
        2 => ['pipe', 'w'],
...
try {
    $installer = escapeshellarg($root . '/database/vp3-single-install.sql');
    $command = implode(' ', [
    ...
    if ((int) $pdo->query('SELECT COUNT(*) FROM accounts')->fetchColumn() !== 1
        ...
        throw new RuntimeException('The first-administrator database identities were not created exactly once.');
    }
