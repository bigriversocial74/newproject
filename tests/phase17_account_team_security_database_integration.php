...
$expectAuthCode = static function (callable $operation, string $code, string $message) use (&$failures): void {
    try {
        ...
        if ($exception->publicCode() !== $code) $failures[] = $message . ' Received ' . $exception->publicCode() . '.';
    }
};
...
$base32Decode = static function (string $value): string {
    ...
    foreach (str_split($clean) as $character) {
        ...
        if ($position === false) throw new RuntimeException('Invalid base32 secret.');
...
try {
    ...
    $createAccount = static function (string $suffix) use ($pdo, $upper, $now, &$accountIds): int {
        $pdo->prepare("INSERT INTO accounts (public_id,account_type,status,display_name,created_at,updated_at) VALUES (:public,'organization','active',:name,:now,:now)")
            ->execute(['public' => 'VP3-P17-' . $upper . '-' . strtoupper($suffix), 'name' => 'Phase 17 ' . ucfirst($suffix), 'now' => $now]);
    ...
    $createUser = static function (string $suffix, string $email) use ($pdo, $upper, $now, $passwordHash, &$userIds): array {
        ...
        $pdo->prepare("INSERT INTO users (public_id,email,email_normalized,password_hash,display_name,status,email_verified_at,created_at,updated_at) VALUES (:public,:email,:email,:hash,:name,'active',:now,:now,:now)")
            ->execute(['public' => $public, 'email' => $email, 'hash' => $passwordHash, 'name' => 'Phase 17 ' . ucfirst($suffix), 'now' => $now]);
        $id = (int) $pdo->lastInsertId();
        $userIds[] = $id;
    ...
    $assert($encrypted['ciphertext'] !== base64_encode('PHASE17-SECRET') && !str_contains($encrypted['ciphertext'], 'PHASE17-SECRET'), 'Auth secret cipher exposed plaintext.');
    $assert($cipher->decrypt($encrypted['ciphertext'], $encrypted['nonce'], $encrypted['tag'], 'phase17:context:a') === 'PHASE17-SECRET', 'Auth secret cipher could not decrypt with the correct context.');
    try {
    ...
    $invite = $team->invite($accountA, $owner['id'], 'customer_owner', $invited['email'], 'support_member', 'REQ-P17-INVITE-A-' . $upper);
    $message = $mail->lastMessage();
    $assert(is_array($message) && str_contains($message['text_body'], 'team-invite.php?token='), 'Invitation delivery did not contain an acceptance token.');
    ...
    $assert($hash === hash('sha256', $inviteToken) && !hash_equals($hash, $inviteToken), 'Invitation token was not stored as a one-way hash.');
    ...
    $denied = $pdo->prepare("SELECT COUNT(*) FROM account_security_receipts WHERE account_id=:account AND action='team.invitation_accepted' AND request_id=:request AND result='denied'");
    $denied->execute(['account' => $accountA, 'request' => $wrongRequest]);
    $assert((int) $denied->fetchColumn() === 1, 'Invitation email-mismatch evidence did not persist.');

    $acceptedAccount = $team->acceptInvitation($invited['id'], $invited['email'], $inviteToken, 'REQ-P17-ACCEPT-' . $upper);
    $assert($acceptedAccount === $accountA, 'Invitation acceptance returned the wrong account.');
    $membership = $pdo->prepare('SELECT role,status FROM account_users WHERE account_id=:account AND user_id=:user');
    $membership->execute(['account' => $accountA, 'user' => $invited['id']]);
    $membershipRow = $membership->fetch(PDO::FETCH_ASSOC);
    $assert(is_array($membershipRow) && $membershipRow['role'] === 'support_member' && $membershipRow['status'] === 'active', 'Invitation acceptance did not create the expected membership.');
    ...
    $expectAuthCode(
        static fn () => $team->acceptInvitation($invited['id'], $invited['email'], $inviteToken, 'REQ-P17-REPLAY-' . $upper),
        'team_invitation_invalid',
        'An accepted invitation token was replayed.'
    );

    $expiredInvite = $team->invite($accountA, $owner['id'], 'customer_owner', $expired['email'], 'support_member', 'REQ-P17-INVITE-X-' . $upper);
    ...
    $pdo->prepare('UPDATE account_invitations SET expires_at=DATE_SUB(UTC_TIMESTAMP(),INTERVAL 1 MINUTE) WHERE public_id=:public')
    ...
    $denied->execute(['account' => $accountA, 'request' => $expiredRequest]);
    $assert((int) $denied->fetchColumn() === 1, 'Expired invitation denial evidence did not persist.');

    $expectAuthCode(
        static fn () => $team->changeRole($accountA, $owner['id'], 'customer_owner', $owner['public_id'], 'customer_admin', 'REQ-P17-FINAL-' . $upper),
    ...
    $team->changeRole($accountA, $owner['id'], 'customer_owner', $admin['public_id'], 'customer_owner', 'REQ-P17-PROMOTE-' . $upper);
    $team->setMembershipStatus($accountA, $owner['id'], 'customer_owner', $admin['public_id'], 'suspended', 'REQ-P17-SUSPEND-' . $upper);
    $revoked = $pdo->prepare('SELECT revoked_at,revocation_reason FROM auth_sessions WHERE session_public_id=:public');
    $revoked->execute(['public' => $session['public_id']]);
    $revokedRow = $revoked->fetch(PDO::FETCH_ASSOC);
    $assert(is_array($revokedRow) && $revokedRow['revoked_at'] !== null && $revokedRow['revocation_reason'] === 'membership_suspended', 'Suspending a member did not revoke active sessions.');

    $enrollment = $mfa->beginEnrollment($owner['id'], $owner['email'], 'Phase 17 Owner', 'REQ-P17-MFA-BEGIN-' . $upper);
    $storedMethod = $pdo->prepare('SELECT secret_ciphertext,secret_nonce,secret_tag,secret_key_id FROM auth_mfa_methods WHERE user_id=:user');
    $storedMethod->execute(['user' => $owner['id']]);
    $methodRow = $storedMethod->fetch(PDO::FETCH_ASSOC);
    $assert(is_array($methodRow) && !str_contains((string) $methodRow['secret_ciphertext'], $enrollment['secret']) && $methodRow['secret_key_id'] === 'phase17-test-key', 'MFA enrollment did not store encrypted key-identified secret evidence.');

    $counter = intdiv(time(), 30);
    $validCodes = [$totp($enrollment['secret'], $counter - 1), $totp($enrollment['secret'], $counter), $totp($enrollment['secret'], $counter + 1)];
    $invalidCode = '000000';
    foreach (['000000','111111','222222','333333','444444','555555','666666','777777','888888','999999'] as $candidate) {
        if (!in_array($candidate, $validCodes, true)) { $invalidCode = $candidate; break; }
    }
    $invalidEnrollRequest = 'REQ-P17-MFA-DENIED-' . $upper;
    $expectAuthCode(
        static fn () => $mfa->confirmEnrollment($owner['id'], $invalidCode, $invalidEnrollRequest),
        'mfa_code_invalid',
        'Invalid MFA enrollment code was accepted.'
    );
    $mfaDenied = $pdo->prepare("SELECT COUNT(*) FROM account_security_receipts WHERE actor_user_id=:user AND action='mfa.enrollment_confirmed' AND request_id=:request AND result='denied'");
    $mfaDenied->execute(['user' => $owner['id'], 'request' => $invalidEnrollRequest]);
    $assert((int) $mfaDenied->fetchColumn() === 1, 'Denied MFA enrollment evidence did not persist.');

    $confirmation = $mfa->confirmEnrollment($owner['id'], $validCodes[1], 'REQ-P17-MFA-CONFIRM-' . $upper);
    $assert($confirmation['enabled'] === true && count($confirmation['recovery_codes']) === 8, 'MFA enrollment did not return the configured recovery codes.');
    ...
    $recoveryUsed = $pdo->prepare('SELECT used_at FROM auth_mfa_recovery_codes WHERE user_id=:user AND code_hash=:hash');
    $recoveryUsed->execute(['user' => $owner['id'], 'hash' => hash('sha256', str_replace('-', '', $recoveryCode))]);
    ...
    $expectAuthCode(
        static fn () => $mfa->completeChallenge($reuseChallenge['challenge_token'], $recoveryCode, '198.51.100.17', 'Phase17-MFA-Agent', $reuseRequest),
        'mfa_code_invalid',
        'A consumed recovery code was reused.'
    );
    $mfaDenied->execute(['user' => $owner['id'], 'request' => $reuseRequest]);
    ...
    $expectAuthCode(
        static fn () => $mfa->completeChallenge($totpReplayChallenge['challenge_token'], $validCodes[1], '198.51.100.17', 'Phase17-MFA-Agent', 'REQ-P17-MFA-TOTP-REPLAY-' . $upper),
        'mfa_code_invalid',
    ...
    $assert($ownerSnapshot['can_manage_team'] === true && count($ownerSnapshot['members']) >= 3 && count($ownerSnapshot['invitations']) >= 2, 'Owner security overview omitted authorized team information.');
    $assert($supportSnapshot['can_manage_team'] === false && count($supportSnapshot['members']) === 1 && $supportSnapshot['members'][0]['current_user'] === true && $supportSnapshot['invitations'] === [], 'Support-member security overview leaked team information.');
} catch (Throwable $exception) {
    $failures[] = $exception::class . ': ' . $exception->getMessage();
} finally {
    if ($userIds !== [] || $accountIds !== []) {
        if ($userIds !== []) {
            ...
            foreach (['auth_mfa_recovery_codes','auth_mfa_challenges','auth_mfa_methods','auth_session_events','auth_sessions'] as $table) {
                $column = $table === 'auth_session_events' ? 'user_id' : 'user_id';
                $pdo->prepare("DELETE FROM {$table} WHERE {$column} IN ({$marks})")->execute($userIds);
            }
            $pdo->prepare("DELETE FROM account_security_receipts WHERE actor_user_id IN ({$marks}) OR target_user_id IN ({$marks})")->execute([...$userIds, ...$userIds]);
