...
final class ProviderSecretCipher
{
    public function __construct(
        ...
        private readonly string $keyId = 'provider-aes256gcm-v1'
    ) {
    }
    ...
    public function encrypt(string $plaintext, string $context): array
    {
        ...
        if (!is_string($ciphertext) || strlen($tag) !== 16) {
            throw new RuntimeException('Provider secret encryption failed.');
        }
        return [
    ...
    public function decrypt(string $ciphertext, string $nonce, string $tag, string $context): string
    {
        ...
        if (!is_string($ciphertextBytes) || !is_string($nonceBytes) || !is_string($tagBytes)) {
            throw new RuntimeException('Provider secret encoding is invalid.');
        }
    ...
    private function key(): string
    {
        ...
        if (!is_string($key) || strlen($key) !== 32) {
            throw new RuntimeException('A valid 32-byte provider secret encryption key is required.');
        }
        return $key;
    }
}
