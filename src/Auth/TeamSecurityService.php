...
final class TeamSecurityService
{
    ...
    public function overview(int $accountId, int $currentUserId, string $currentSessionPublicId): array
    {
        ...
        $currentMembership = array_values(array_filter($memberRows, static fn (array $row): bool => $row['current_user']));
        return [
    ...
    public function invite(int $accountId, int $actorUserId, string $actorRole, string $email, string $role, string $requestId): array
    {
        ...
        $accountName = '';
        $this->database->transaction(function (PDO $pdo) use ($accountId, $actorUserId, $email, $role, $requestId, $token, $publicId, $now, $expires, &$accountName): void {
            $account = $pdo->prepare('SELECT display_name FROM accounts WHERE id=:account AND status=\'active\' LIMIT 1 FOR UPDATE');
            $account->execute(['account' => $accountId]);
            $accountName = (string) $account->fetchColumn();
            if ($accountName === '') throw new RuntimeException('The active account was not found.');
            $existing = $pdo->prepare(
                "SELECT 1 FROM account_users au JOIN users u ON u.id=au.user_id
                 WHERE au.account_id=:account AND u.email_normalized=:email AND au.status IN ('active','invited','suspended') LIMIT 1"
            );
            $existing->execute(['account' => $accountId, 'email' => $email]);
            if ($existing->fetchColumn()) throw new AuthPublicException('team_member_exists', 'This email already belongs to the account team.', 409);
            $pdo->prepare(
            ...
            )->execute(['now' => $now->format('Y-m-d H:i:s'), 'account' => $accountId, 'email' => $email]);
            $pdo->prepare(
                'INSERT INTO account_invitations
                 ...
                 VALUES (:public,:account,:email,:normalized,:role,:hash,\'pending\',:actor,NULL,:request,:expires,NULL,NULL,:now,:now)'
            )->execute(['public' => $publicId, 'account' => $accountId, 'email' => $email, 'normalized' => $email, 'role' => $role,
                'hash' => hash('sha256', $token), 'actor' => $actorUserId, 'request' => $requestId,
                'expires' => $expires->format('Y-m-d H:i:s'), 'now' => $now->format('Y-m-d H:i:s')]);
            $this->receipt($pdo, $accountId, $actorUserId, null, 'team.invited', 'success', $requestId, ['invitation_public_id' => $publicId, 'role' => $role, 'email_hash' => hash('sha256', $email)]);
            $this->audit->record('team.invited', 'success', $actorUserId, $accountId, 'account_invitation', $publicId, ['role' => $role, 'email_hash' => hash('sha256', $email)], $requestId);
        });
        ...
        try {
            $this->mail->send(
                ...
                '<p>You were invited to <strong>' . htmlspecialchars($accountName, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</strong> on VP3.me.</p><p><a href="' . htmlspecialchars($url, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '">Accept invitation</a></p><p>This invitation expires at ' . htmlspecialchars($expires->format(DATE_ATOM), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '.</p>'
            );
        } catch (Throwable) {
            throw new AuthPublicException('team_invitation_delivery_failed', 'The invitation was created, but email delivery failed. Revoke it and try again.', 503);
        }
        ...
        return $this->database->transaction(function (PDO $pdo) use ($userId, $userEmail, $token, $requestId): int {
            ...
            $pdo->prepare(
                "INSERT INTO account_users (account_id,user_id,role,status,created_at,updated_at)
                 VALUES (:account,:user,:role,'active',:now,:now)
                 ON DUPLICATE KEY UPDATE role=VALUES(role),status='active',updated_at=VALUES(updated_at)"
            )->execute(['account' => $invite['account_id'], 'user' => $userId, 'role' => $invite['role'], 'now' => $now->format('Y-m-d H:i:s')]);
            $pdo->prepare(
                "UPDATE account_invitations SET status='accepted',accepted_by_user_id=:user,accepted_at=:now,updated_at=:now WHERE id=:id AND status='pending'"
    ...
    public function revokeInvitation(int $accountId, int $actorUserId, string $publicId, string $requestId): void
    {
        $this->database->transaction(function (PDO $pdo) use ($accountId, $actorUserId, $publicId, $requestId): void {
            ...
            $statement->execute(['now' => $now, 'public' => $publicId, 'account' => $accountId]);
            if ($statement->rowCount() !== 1) throw new RuntimeException('The pending invitation was not found.');
            $this->receipt($pdo, $accountId, $actorUserId, null, 'team.invitation_revoked', 'success', $requestId, ['invitation_public_id' => $publicId]);
            $this->audit->record('team.invitation_revoked', 'success', $actorUserId, $accountId, 'account_invitation', $publicId, [], $requestId);
        });
    }

    public function changeRole(int $accountId, int $actorUserId, string $actorRole, string $targetPublicId, string $role, string $requestId): void
    {
        $this->assertRoleAssignment($actorRole, $role);
        $this->mutateMembership($accountId, $actorUserId, $actorRole, $targetPublicId, $requestId, 'role_changed', function (PDO $pdo, array $target) use ($role): void {
            if ((string) $target['role'] === 'customer_owner' && $role !== 'customer_owner') $this->assertAnotherOwner($pdo, (int) $target['account_id'], (int) $target['user_id']);
            $pdo->prepare('UPDATE account_users SET role=:role,updated_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['role' => $role, 'id' => $target['membership_id']]);
        }, ['role' => $role]);
    }

    public function setMembershipStatus(int $accountId, int $actorUserId, string $actorRole, string $targetPublicId, string $status, string $requestId): void
    {
        if (!in_array($status, ['active', 'suspended', 'removed'], true)) throw new AuthPublicException('team_status_invalid', 'The requested membership status is invalid.', 422);
        $this->mutateMembership($accountId, $actorUserId, $actorRole, $targetPublicId, $requestId, 'status_changed', function (PDO $pdo, array $target) use ($status, $actorUserId): void {
            if ((int) $target['user_id'] === $actorUserId && $status !== 'active') throw new AuthPublicException('team_self_removal_denied', 'Use another active owner to remove or suspend your membership.', 409);
            if ((string) $target['role'] === 'customer_owner' && $status !== 'active') $this->assertAnotherOwner($pdo, (int) $target['account_id'], (int) $target['user_id']);
            $pdo->prepare('UPDATE account_users SET status=:status,updated_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['status' => $status, 'id' => $target['membership_id']]);
            if ($status !== 'active') {
                $pdo->prepare("UPDATE auth_sessions SET revoked_at=UTC_TIMESTAMP(),revocation_reason='membership_{$status}',revoked_by_user_id=:actor,updated_at=UTC_TIMESTAMP() WHERE user_id=:user AND revoked_at IS NULL")
                    ->execute(['actor' => $actorUserId, 'user' => $target['user_id']]);
            }
        }, ['status' => $status]);
    }

    /** @param callable(PDO,array<string,mixed>):void $mutation @param array<string,mixed> $metadata */
    private function mutateMembership(int $accountId, int $actorUserId, string $actorRole, string $targetPublicId, string $requestId, string $action, callable $mutation, array $metadata): void
    {
        $this->database->transaction(function (PDO $pdo) use ($accountId, $actorUserId, $actorRole, $targetPublicId, $requestId, $action, $mutation, $metadata): void {
            $statement = $pdo->prepare(
            ...
            if (!is_array($target)) throw new RuntimeException('The account member was not found.');
            if ($actorRole !== 'customer_owner' && (string) $target['role'] === 'customer_owner') throw new AuthPublicException('team_owner_change_denied', 'Only an account owner can modify another owner.', 403);
            $mutation($pdo, $target);
            $event = 'team.' . $action;
            $evidence = array_merge($metadata, ['target_public_id' => $targetPublicId]);
            $this->receipt($pdo, $accountId, $actorUserId, (int) $target['user_id'], $event, 'success', $requestId, $evidence);
            $this->audit->record($event, 'success', $actorUserId, $accountId, 'account_member', $targetPublicId, $evidence, $requestId);
        });
    }

    private function assertRoleAssignment(string $actorRole, string $role): void
    {
        if (!in_array($role, self::CUSTOMER_ROLES, true)) throw new AuthPublicException('team_role_invalid', 'The requested account role is invalid.', 422);
        if ($role === 'customer_owner' && $actorRole !== 'customer_owner') throw new AuthPublicException('team_owner_assignment_denied', 'Only an account owner can assign the owner role.', 403);
        if (!in_array($actorRole, ['customer_owner', 'customer_admin'], true)) throw new AuthPublicException('team_permission_denied', 'The current account role cannot manage team members.', 403);
    }

    private function assertAnotherOwner(PDO $pdo, int $accountId, int $excludedUserId): void
    {
        $statement = $pdo->prepare("SELECT COUNT(*) FROM account_users WHERE account_id=:account AND user_id<>:excluded AND role='customer_owner' AND status='active'");
        $statement->execute(['account' => $accountId, 'excluded' => $excludedUserId]);
        if ((int) $statement->fetchColumn() < 1) throw new AuthPublicException('team_final_owner_required', 'The account must retain at least one other active owner.', 409);
    }
    ...
    private function receipt(PDO $pdo, ?int $accountId, ?int $actorUserId, ?int $targetUserId, string $action, string $result, string $requestId, array $metadata): void
    {
        ...
        $pdo->prepare('INSERT INTO account_security_receipts (public_id,account_id,actor_user_id,target_user_id,action,result,request_id,evidence_hash,created_at) VALUES (:public,:account,:actor,:target,:action,:result,:request,:hash,UTC_TIMESTAMP())')
            ->execute(['public' => 'SEC-' . strtoupper(bin2hex(random_bytes(10))), 'account' => $accountId, 'actor' => $actorUserId, 'target' => $targetUserId, 'action' => $action, 'result' => $result, 'request' => $requestId, 'hash' => hash('sha256', json_encode($evidence, JSON_THROW_ON_ERROR))]);
    }
