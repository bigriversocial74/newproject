...
final class AccountBillingQueryService
{
    ...
    private function account(int $accountId): array
    {
        ...
        return ['id' => (int) $row['id'], 'public_id' => (string) $row['public_id'], 'display_name' => (string) $row['display_name'], 'status' => (string) $row['status']];
    }
    ...
    private function plans(): array
    {
        $statement = $this->database->pdo()->query(
            "SELECT p.public_id,p.code,p.name,p.billing_interval,p.currency,p.price_minor,
                    sp.billing_interval AS checkout_interval,sp.currency AS checkout_currency,sp.unit_amount AS checkout_amount,
    ...
    {
        $statement = $this->database->pdo()->prepare(
            "SELECT status,billing_reason,currency,amount_due,amount_paid,amount_remaining,hosted_invoice_url,invoice_pdf_url,
                    period_start,period_end,due_at,paid_at,created_at,updated_at
             FROM billing_invoices WHERE account_id=:account ORDER BY created_at DESC,id DESC LIMIT 50"
        );
        $statement->execute(['account' => $accountId]);
        return array_map(fn (array $row): array => [
            ...
            'period_start' => $row['period_start'], 'period_end' => $row['period_end'], 'due_at' => $row['due_at'],
            'paid_at' => $row['paid_at'], 'created_at' => $row['created_at'], 'updated_at' => $row['updated_at'],
        ], $statement->fetchAll(PDO::FETCH_ASSOC));
    }
    ...
    private function payments(int $accountId): array
    {
        $statement = $this->database->pdo()->prepare(
            "SELECT status,currency,amount,amount_received,payment_method_type,failure_code,failure_message,created_at,updated_at
             FROM billing_payment_intents WHERE account_id=:account ORDER BY created_at DESC,id DESC LIMIT 30"
        );
        $statement->execute(['account' => $accountId]);
        return array_map(static fn (array $row): array => [
            'status' => (string) $row['status'], 'currency' => strtoupper((string) $row['currency']),
            'amount' => (int) $row['amount'], 'amount_received' => (int) $row['amount_received'],
    ...
    private function refunds(int $accountId): array
    {
        $statement = $this->database->pdo()->prepare(
            "SELECT status,currency,amount,reason,failure_reason,created_at,updated_at
             FROM billing_refunds WHERE account_id=:account ORDER BY created_at DESC,id DESC LIMIT 30"
        );
        $statement->execute(['account' => $accountId]);
        return array_map(static fn (array $row): array => [
            'status' => (string) $row['status'], 'currency' => strtoupper((string) $row['currency']), 'amount' => (int) $row['amount'],
    ...
    private function attention(array $subscriptions, array $invoices, array $payments): array
    {
        ...
        foreach ($subscriptions as $subscription) {
            if (in_array($subscription['status'], ['past_due', 'grace'], true)) {
                $items[] = ['severity' => $subscription['status'] === 'past_due' ? 'critical' : 'warning',
                    'title' => $subscription['status'] === 'past_due' ? 'Subscription payment is past due' : 'Subscription is in a billing grace period',
                    'detail' => (string) $subscription['plan']['name'] . ($subscription['grace_ends_at'] ? ' · Grace ends ' . $subscription['grace_ends_at'] : ''), 'action' => 'portal'];
            }
        }
        foreach ($invoices as $invoice) {
            if ((int) $invoice['amount_remaining'] > 0 && !in_array($invoice['status'], ['paid', 'void'], true)) {
                $items[] = ['severity' => in_array($invoice['status'], ['uncollectible', 'open'], true) ? 'critical' : 'warning',
                    'title' => 'Invoice requires attention',
                    'detail' => strtoupper((string) $invoice['currency']) . ' ' . number_format(((int) $invoice['amount_remaining']) / 100, 2), 'action' => 'portal'];
            }
        }
        foreach ($payments as $payment) {
            if (in_array($payment['status'], ['requires_payment_method', 'requires_action', 'canceled', 'failed'], true)) {
                $items[] = ['severity' => 'critical', 'title' => 'Payment attempt failed',
                    'detail' => $payment['failure_message'] ?: 'Update the payment method in the secure billing portal.', 'action' => 'portal'];
            }
        }
    ...
    {
        if ($invoices !== []) return strtoupper((string) $invoices[0]['currency']);
        if ($plans !== []) return strtoupper((string) $plans[0]['currency']);
        return 'USD';
    }
    ...
    private function trustedStripeUrl(mixed $value): ?string
    {
        ...
        if ($scheme !== 'https' || ($host !== 'stripe.com' && !str_ends_with($host, '.stripe.com'))) return null;
        return $url;
    }
}
