...
try {
    if (!is_file($configPath)) {
        ...
        $baseUrlParts = parse_url((string) ($app['base_url'] ?? ''));
        ...
        if (!$setupEnabled
            ...
            $pdo = $database->pdo();
            $requiredTables = [
                'accounts', 'users', 'account_users', 'platform_deployment_receipts',
                'platform_operator_accounts', 'platform_release_control_receipts',
            ];
            $missingTables = [];
            ...
            } elseif ((int) $pdo->query('SELECT COUNT(*) FROM accounts')->fetchColumn() !== 0
...
<p>This one-time page creates the first organization owner and grants platform administration.</p>
...
<div class="field"><label for="password">Password</label><input id="password" name="password" type="password" required minlength="<?= (int) (($config['auth']['password_min_length'] ?? 12)) ?>" autocomplete="new-password"></div>
<div class="field"><label for="password_confirmation">Confirm password</label><input id="password_confirmation" name="password_confirmation" type="password" required minlength="<?= (int) (($config['auth']['password_min_length'] ?? 12)) ?>" autocomplete="new-password"></div>
</div><button class="button" type="submit">Create first administrator</button></form>
