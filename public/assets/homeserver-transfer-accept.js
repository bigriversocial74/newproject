(() => {
  ...
  function showBundle(bundle) {
    const bundleAccountId = accountPublicId();
    ...
    modal.querySelector("#copy-transfer-bundle")?.addEventListener("click", async () => {
      await navigator.clipboard.writeText(JSON.stringify({ account_public_id: bundleAccountId, device_public_id: bundle.device_public_id, credential: bundle.credential, enrollment_code: bundle.enrollment_code }, null, 2));
    });
