...
try {
    if (!is_file($configPath)) {
        ...
        if (!$setupEnabled || strlen($setupKey) < 20 || str_contains(strtoupper($setupKey), 'CHANGE_THIS')) {
            $state = 'config_invalid';
            $message = 'Enable browser setup and replace the example setup key in config/config.php with a private value of at least 20 characters.';
        } else {
